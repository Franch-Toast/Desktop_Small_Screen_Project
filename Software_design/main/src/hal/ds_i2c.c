...
esp_err_t i2c_master_read_slave(uint8_t u8Cmd, uint8_t *data_rd, size_t size)
{
    ...
    i2c_master_read_byte(cmd, data_rd + size - 1, NACK_VAL);//最后一位无需应答
 ...
 */
esp_err_t i2c_master_init(void) // I2C端口主机配置
{
    int i2c_master_port = I2C_MASTER_NUM; // 主机端口号设置
    i2c_config_t conf;
    conf.mode = I2C_MODE_MASTER; // 配置为主机
    conf.sda_io_num = I2C_MASTER_SDA_IO;
    conf.sda_pullup_en = GPIO_PULLUP_ENABLE; // 启用 ESP32 的内部上拉电阻
    conf.scl_io_num = I2C_MASTER_SCL_IO;
    conf.scl_pullup_en = GPIO_PULLUP_ENABLE;    // 启用 ESP32 的内部上拉电阻
    conf.master.clk_speed = I2C_MASTER_FREQ_HZ; // I2C 时钟速度
