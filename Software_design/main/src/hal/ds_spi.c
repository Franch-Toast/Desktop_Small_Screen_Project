...
#define PIN_NUM_MISO 33 // 屏幕只写不读，此引脚其实是无效的
...
void spi_send_cmd(const uint8_t cmd)
{
    esp_err_t ret;
    spi_transaction_t t;
    ds_gpio_set_screen_dc(0); // 设置发送指令
    ds_gpio_set_screen_cs(0); // 设置片选模块
    ...
    ds_gpio_set_screen_cs(1);                   // 取消片选模块
...
void spi_send_data(const uint8_t data)
{
    esp_err_t ret;
    spi_transaction_t t;
    ds_gpio_set_screen_dc(1); // 设置发送数据
    ds_gpio_set_screen_cs(0); // 设置片选模块
    memset(&t, 0, sizeof(t)); // Zero out the transaction
